...
class Transformer(nn.Module):
    ...
    def unfold_ours(self, tensor):
        ...
        tensor = F.unfold(tensor, kernel_size=(self.fold_k, self.fold_k), padding=self.fold_p,
                          stride=self.fold_stride)  # [B,C*k*k,out_h*out_w]
        #         print("tensor.shape:",tensor.shape,B,C,self.fold_k,self.fold_k,out_h,out_w)
        tensor = tensor.reshape(B, C, self.fold_k, self.fold_k, out_h, out_w).permute(0, 1, 4, 5, 2, 3).reshape(B,
                                                                                                                C * out_h * out_w,
                                                                                                                self.fold_k * self.fold_k)
        return tensor
    ...
    def forward(self, style_src, mask, src, query_pos_embed, pos_embed):
        ...
        query_pos_embed = query_pos_embed.flatten(2).permute(2, 0,
                                                             1) if query_pos_embed is not None else None  # [H*W,B,C]
        mask = mask.flatten(1)
        #         print("mask:",mask.shape)
        #         tgt = torch.zeros_like(query_embed)
        tgt = src
...
def show_feature_map(feature_map, verbose="", out_root="", norm=False):
    ...
    for index in range(feature_map_num)[:50]:
        ...
        #         plt.axis('off')
        imageio.imwrite(os.path.join(out_root, "{}_{}.png".format(index, verbose)),
                        feature_map[index].cpu().detach().numpy())
